...
import type { StorageOperationResult, StoredSong } from "@/types/offline";
 ...
 */
export async function getSong(slug: string): Promise<StoredSong | undefined> {
  try {
    const db = await initDB();
...
export async function saveMultipleSongs(
  ...
  try {
    ...
    await Promise.all([...songs.map((song) => tx.store.put(song)), tx.done]);
...
export async function removeMultipleSongs(
  ...
  try {
    ...
    await Promise.all([...slugs.map((slug) => tx.store.delete(slug)), tx.done]);
